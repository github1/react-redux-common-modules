...
describe('when connecting modules', () => {
  it('uses a default mapStateToProps function', () => {
    ...
    const res = TestRenderer.create(createElement(TestComponentConnected, { store: TestModule.asStore() }));
  ...
  it('ownProps take precendence with default mapStateToProps function', () => {
    const TestComponentConformsToModuleState: React.FC<{ test: number }> = (
      ...
      return React.createElement('div', {}, `inside connected comp: ${props.test}`);
    };
    const TestModule = createModule('root').reduce((state: { test: number }) => {
      return state;
    }).preloadedState({
      test: 55
    });
    ...
    const res = TestRenderer.create(createElement(TestComponentConnected, { test: 12, store: TestModule.asStore() }));
  ...
  it('can be provided a mapStateToProps function', () => {
    ...
    const TestComponent: React.FC<TestComponentProps> = (props) => {
      return React.createElement('div', {}, `${props.componentProp}-${props.anotherComponentProp}`);
    };
    ...
    const TestComponentConnected = connectModule(TestModule, TestComponent, {
    ...
    const res = TestRenderer.create(createElement(TestComponentConnected, { anotherComponentProp: 'from-own-props', store: TestModule.asStore() }));
    expect(res.toJSON().children[0]).toBe('from-store-state-another-from-own-props');
  });
  it('can be map dispatch to actions', () => {
    const TestComponentConformsToModuleState: React.FC<{
      ...
      return React.createElement('div', {}, `${props.test}-${props.doSomething.toString().replace(/\n|[ ]+/g,'')}`);
    };
    const TestModule = createModule(
      'root', {
        actionCreators: {
    ...
    >({
      test: 'string',
      doSomething: (val: number) => { if(val) {} },
    });
    const res = TestRenderer.create(createElement(TestComponentConnected, { store: TestModule.asStore() }));
    expect(res.toJSON().children[0]).toContain('from-store-state-function(){returndispatch(actionCreator.apply(this,arguments));}');
  });
});
