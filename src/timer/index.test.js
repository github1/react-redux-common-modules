import timer, { startTimer, stopTimer, TIMER_TICK, TIMER_STOPPED } from './index';
...
describe('timer', () => {
    ...
    it('can invoke actions on tick', () => {
        ...
        return new Promise((resolve,reject) => {
            store.dispatch(startTimer('timer1', {interval: 1, action: { type: 'TestAction' }}));
    ...
    it('can stop the timer before a tick', () => {
        let maxTicks = 5;
        return new Promise((resolve,reject) => {
            store.dispatch(startTimer('timer1', {interval: 99, action: { type: 'TestAction' }, dispatchOnTick: 2}));
            const check = setInterval(() => {
                maxTicks--;
                if (maxTicks === 4) {
                    store.dispatch(stopTimer('timer1'));
