...
export interface DataFetchRequestBuilder<T, A = any, K extends string = string> extends DataFetchRequested {
  fromQuery(graphQueryDefinition : DataFetchQueryDefinition<T, A, K>) : DataFetchRequestBuilder<T, A, K>;
  ...
  withPostProcessor(postProcessor : DataFetchPostProcessor<T>) : DataFetchRequestBuilder<T, A, K>;
...
function applyDataFetchPostProcessor(postProcessor: DataFetchPostProcessor<any>, data: any, state: any): any {
...
export const dataFetch = <T, A = any, K extends string = string>(name? : string) : DataFetchRequestBuilder<T, A, K> => {
  ...
  const dataFetch : DataFetchRequestBuilder<T, A, K> = {
    ...
    url: undefined,
    fromQuery(graphQueryDefinition : DataFetchQueryDefinition<T, A, K>) : DataFetchRequestBuilder<T, A, K> {
      const dataFetchQueryNamedDefinition : DataFetchQueryNamedDefinition<K, T, A> = graphQueryDefinition as DataFetchQueryNamedDefinition<K, T, A>;
      dataFetch.graphQuery = dataFetchQueryNamedDefinition.schema ? dataFetchQueryNamedDefinition.schema : graphQueryDefinition;
      dataFetch.postProcessor = dataFetchQueryNamedDefinition.postProcessor as DataFetchPostProcessor<any>;
      dataFetch.queryResultName = Object.keys(dataFetch.graphQuery)[0];
      dataFetch.queryName = (dataFetchQueryNamedDefinition.name || dataFetch.queryResultName) as string;
      return dataFetch;
    },
    fromStaticData(data : any) : DataFetchRequestBuilder<T, A, K> {
  ...
  middleware: store => next => action => {
    if (AUTHENTICATE_REQUESTED === action.type) {
      ...
      if (action.staticData) {
        const data = applyDataFetchPostProcessor(action.postProcessor, action.staticData, store.getState());
        ...
        store.dispatch(doGetRequest(action.queryName, action.url, store.getState(), (err, response) => {
          if (err) {
            ...
            const data = applyDataFetchPostProcessor(action.postProcessor, response.data, store.getState());
        ...
        store.dispatch(doGraphQuery(action.queryName, action.tag, action.graphQuery, store.getState(), (err, response) => {
          try {
            if (err) {
              ...
              const data = applyDataFetchPostProcessor(action.postProcessor, response.data['graph'][action.queryResultName], store.getState());
              return dataFetchSuccess({
                dataFetchId: action.dataFetchId,
                queryName: action.queryName,
